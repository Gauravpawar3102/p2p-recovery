...
import { getSmartAccountData } from '@/lib/storage';
import {
  ERC20_ABI,
  SMART_ACCOUNT_ABI,
  ENTRY_POINT_ABI,
...
export function TokenTransfer({ network }: TokenTransferProps) {
  ...
  const fetchTokens = async (method: 'free' | 'paid' = 'free') => {
    ...
    try {
      const endpoint =
        method === 'free'
          ? '/api/tokenfetch/nativeandusdc'
          : '/api/tokenfetch/otherstoken';
      const networkConfig = NETWORKS[network];
      ...
      const usdcToken = fetchedTokens.find(
        (t: TokenInfo) =>
      ...
      if (fetchedTokens.length > 0) {
        console.log(
          `Found ${fetchedTokens.length} token(s) using ${method === 'free' ? 'Native & USDC' : 'Zapper API'}`,
        );
      }
    } catch (err: any) {
  ...
  const handleTransfer = async () => {
    ...
    const finalRecipient =
      transferMode === 'recover' ? ownerAddress : recipient;
    ...
    if (transferAmount > currentBalance) {
      ...
      setError(
        `Insufficient balance. You have ${displayBalance} ${tokenToTransfer.symbol}`,
      );
      return;
    }
    ...
    try {
      ...
      try {
        const gasPrices = await bundlerRpc(
      ...
      try {
        const gasEstimate = await bundlerRpc(
          ...
          [formatUserOpForBundler(userOp), networkConfig.entryPoint],
          networkConfig.bundlerUrl,
        );

        if (gasEstimate) {
          userOp.callGasLimit = BigInt(gasEstimate.callGasLimit || '0x493e0');
          userOp.verificationGasLimit = BigInt(
            gasEstimate.verificationGasLimit || '0x493e0',
          );
          // Ensure preVerificationGas is at least 600000 or the estimated value, whichever is higher
          const estimatedPreVerificationGas = BigInt(
            gasEstimate.preVerificationGas || '0x927c0',
          );
          userOp.preVerificationGas =
            estimatedPreVerificationGas > 600000n
      ...
      try {
        ...
        console.warn(
          'Failed to get paymaster data, user will pay gas:',
          e.message,
        );
      }
      ...
      if (wallet.getAdminAccount) {
        try {
          ...
          console.warn(
            'Could not get admin account, using connected account:',
            e,
          );
        }
      }
      ...
      const userOpHashResult = await bundlerRpc(
        'eth_sendUserOperation',
        [formattedUserOp, networkConfig.entryPoint],
        networkConfig.bundlerUrl,
      );
      ...
      if (receipt) {
        ...
        const deployMsg = !deployed
          ? ' (Account deployed and transfer completed!)'
          : '';
        const actionMsg =
        ...
        setSuccess(
          `Transaction pending. Check explorer for UserOp: ${userOpHashResult}`,
        );
      }
    } catch (err: any) {
    ...
    <div className='bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-5 md:p-6 shadow-soft'>
      <div className='flex items-center justify-between mb-5 md:mb-6'>
        <div>
          ...
          <h2 className='text-lg md:text-xl font-semibold text-neutral-900 dark:text-neutral-50 mt-2'>
            Token Transfer
          </h2>
        </div>
        {isLoadingTokens && (
      ...
      <div className='space-y-4'>
        ...
        <div>
          ...
          <div className='relative'>
            ...
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.backgroundColor = '#7469ce')
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.backgroundColor = '#8984d9')
              }>
              Max
            </button>
          </div>
        ...
        <button
          ...
          onMouseEnter={(e) =>
            !isLoading && (e.currentTarget.style.backgroundColor = '#7469ce')
          }
          onMouseLeave={(e) =>
            !isLoading && (e.currentTarget.style.backgroundColor = '#8984d9')
          }>
          {isLoading ? (
            <>
      ...
      {error && (
        <div className='flex items-start gap-3 p-4 bg-error-light dark:bg-error-dark/20 border border-error/30 rounded-lg text-error-dark dark:text-error-light mt-4 overflow-hidden'>
          ...
          <p className='text-sm font-medium break-all overflow-wrap-anywhere flex-1'>
            {error}
          </p>
        </div>
      )}
        ...
        <div className='fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          <div className='bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl max-w-md w-full p-6 relative shadow-strong max-h-[90vh] overflow-y-auto animate-fade-in'>
            ...
            <p className='text-center text-sm text-neutral-600 dark:text-neutral-400 mb-5'>
              Your smart account needs native tokens (
              {NETWORKS[network].chain.nativeCurrency.symbol}) to pay for
            ...
            <div className='bg-info-light dark:bg-info-dark/20 border border-info/30 rounded-lg p-4 mb-4'>
              ...
              <ol className='text-sm text-neutral-700 dark:text-neutral-300 space-y-1.5 list-decimal list-inside'>
                <li>
                  Send {NETWORKS[network].chain.nativeCurrency.symbol} to the
                  address above
                </li>
            ...
            <button
              ...
              onMouseEnter={(e) =>
                (e.currentTarget.style.backgroundColor = '#c5c5c6')
              }
              onMouseLeave={(e) =>
                (e.currentTarget.style.backgroundColor = '#d6d6d7')
              }>
              Close
            </button>
          </div>
