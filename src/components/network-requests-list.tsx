...
export function NetworkRequestsList() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const sortedData = data.data.sort(
        (a: NetworkRequest, b: NetworkRequest) => b.count - a.count,
      );
  ...
  if (isLoading) {
    return (
      <div
        className='bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg sm:rounded-xl p-6 sm:p-8 shadow-soft'
        suppressHydrationWarning>
        <div className='flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-3'>
          <Loader2 className='w-5 h-5 animate-spin text-brand-500' />
          <p className='text-xs sm:text-sm md:text-base font-medium text-neutral-900 dark:text-neutral-50'>
  ...
  return (
    <div
      className='bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-lg sm:rounded-xl p-4 sm:p-5 md:p-6 shadow-soft'
      suppressHydrationWarning>
      <div className='flex items-center justify-between mb-4 sm:mb-5'>
        <div>
          <h2 className='text-base sm:text-lg md:text-xl font-semibold text-neutral-900 dark:text-neutral-50'>
            Requested Networks
          </h2>
        </div>
      ...
      {requests.length === 0 ? (
        <div className='text-center py-8 sm:py-10 md:py-12'>
          ...
          <p className='text-sm sm:text-base text-neutral-900 dark:text-neutral-50 font-medium'>
            No network requests yet
          </p>
          <p className='text-xs sm:text-sm text-neutral-500 dark:text-neutral-400 mt-1'>
            Be the first to request a network!
          </p>
        </div>
      ) : (
          ...
          {requests.map((request, index) => (
            <div
              ...
              <div className='flex items-center gap-2.5 sm:gap-3 md:gap-4 flex-1 min-w-0'>
                {/* Rank Badge */}
                <div
                  className={`w-8 h-8 sm:w-10 sm:h-10 border border-neutral-300 dark:border-neutral-600 rounded-lg flex items-center justify-center flex-shrink-0 ${
                    index === 0
                  ...
                  <span className='text-xs sm:text-sm font-semibold text-neutral-900 dark:text-neutral-50'>
                    #{index + 1}
                  </span>
                </div>
              ...
              <div className='flex items-center gap-1.5 sm:gap-2 ml-2 sm:ml-3 flex-shrink-0'>
                <div className='text-right'>
                  <p className='text-base sm:text-lg md:text-xl font-semibold text-neutral-900 dark:text-neutral-50'>
                    {request.count}
                  </p>
