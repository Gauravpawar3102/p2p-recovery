  ...
  try {
    ...
    if (
      ...
      const friendlyError = new Error(
        'Factory not deployed on this network yet',
      );
...
export function getInitCode(
  ...
  const createAccountCallData = encodeFunctionData({
    abi: parseAbi([
      'function createAccount(address owner, bytes data) returns (address)',
    ]),
  ...
  return (factoryAddress.toLowerCase() +
    createAccountCallData.slice(2)) as `0x${string}`;
}
  ...
  try {
    ...
    try {
      const gasEstimate = await bundlerRpc(
        ...
        [formatUserOpForBundler(userOp), network.entryPoint],
        network.bundlerUrl,
      );
      ...
      if (gasEstimate) {
        ...
        userOp.verificationGasLimit = BigInt(
          gasEstimate.verificationGasLimit || '0xf4240',
        );
    ...
    const userOpHashResult = await bundlerRpc(
      ...
      [formattedUserOp, network.entryPoint],
      network.bundlerUrl,
    );
...
export async function deploySmartAccountWithWallet(
  ...
  try {
    ...
    try {
      userOpHashResult = await bundlerRpc(
        'eth_sendUserOperation',
        [formattedUserOp, network.entryPoint],
        network.bundlerUrl,
      );
